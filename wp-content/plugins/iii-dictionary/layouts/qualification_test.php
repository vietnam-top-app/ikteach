<?php
	...
	$math_tab_options = array(
		'items' => array(
			//'register' => array('url' => locale_home_url() . '/?r=qualification-test/register', 'text' => __('Register', 'iii-dictionary')),
			'tutor-math' => array('url' => locale_home_url() . '/?r=qualification-test/tutor-math', 'text' => __('Tutor Math', 'iii-dictionary')),
			//'request-payment' => array('url' => locale_home_url() . '/?r=qualification-test/request-payment', 'text' => __('Request Payment', 'iii-dictionary'))
		),
		'active' => $active_tab
	);
	...
	switch($active_tab)
	{
		...
		case 'register':
			...
			if(isset($_POST['i-agree']) || isset($_POST['update-teacher'])) {
				if(MWDB::update_user($current_user, $is_math_panel)) {
					wp_redirect(locale_home_url() . '/?r=qualification-test');
					exit;
				}
			}
		...
		case 'sell-worksheet':
			...
			if(isset($_POST['remove-offer'])) {
				if(MWDB::remove_offered_worksheet($_POST['cid'])) {
					wp_redirect(locale_home_url() . '/?r=qualification-test/sell-worksheet');
					exit;
				}
			}
		...
		case 'purchase-worksheet':
			...
			if(isset($_POST['purchase-worksheet'])) {
				if(ik_purchase_worksheet($_POST['cid'])) {
					wp_redirect(locale_home_url() . '/?r=qualification-test/purchase-worksheet');
					exit;
				}
			}
		...
		case 'teach-class':
			...
			if(isset($_POST['i-agree'])) {
				...
				wp_redirect(locale_home_url() . '/?r=qualification-test/teach-class');
				exit;
			}
				...
				if($form_valid) {
					...
					if(ik_deduct_user_points($amount, $current_user->ID)) {
						if(MWDB::store_payment_request($data)) {
							...
							wp_redirect(locale_home_url() . '/?r=qualification-test/request-payment');
							exit;
						}
						else {
		...
		case 'tutor-math' : 
			...
			if(isset($_POST['take-math-test1']) || isset($_POST['take-math-test2'])) {
				...
				if(!empty($math_tests->items)) {
					...
					ik_enqueue_messages(__('This test does not have any assignments','iii-dictionary'),'message');
					wp_redirect(locale_home_url() . '/?r=qualification-test/tutor-math');
					exit;
				}
			}
	...
	if((!$is_mw_registered_teacher || (!$is_teaching_agreement_uptodate && !$is_math_panel) || (!$is_teaching_agreement_uptodate_math && $is_math_panel)) && $active_tab != 'register') {
		...
		$return_url = locale_home_url() . '/?r=qualification-test/my-account';
...
?>
	...
	<form action="<?php echo locale_home_url() . '/?r=qualification-test/' . $active_tab ?>" method="post" id="main-form" enctype="multipart/form-data">
		<div class="row">
			<div class="col-sm-12">
				...
				<div class="col-sm-12">
					<div class="step-block">
						...
						<div id="step1-collapse"<?php echo is_mw_qualified_teacher() ? ' class="collapse"' : '' ?>>
							...
							<div class="box box-red">
								<ul> 
									<li><strong>1)</strong> <?php _e('Take the following two tests. Both tests are auto graded and the result will be displayed here. If you fail, you can retake if you wish.', 'iii-dictionary') ?></li>
									<li><strong>2)</strong> <?php _e('Once you cleared both test, the administrator will review the result and notify you if you may teach at this site as a math teacher.', 'iii-dictionary') ?></li>
								</ul>
												...
												<table class="table table-striped table-condensed ik-table1 text-center" id="list-request-chat">
													<thead>
														<tr>
															...
															<th><?php _e('Price /1 minutes', 'iii-dictionary') ?></th>
														</tr>
													</thead>
													<tfoot>
