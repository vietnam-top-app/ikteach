<?php
	$page_title_tag = __('Reading Comprehension Homework', 'iii-dictionary');
	...
	$homework_sheets = MWDB::get_homework_sheets(ASSIGNMENT_READING);
	$sheet_list = $homework_sheets;

	if($mode == 'practice')
	{
		$page_title_tag = __('Reading Comprehension Practice', 'iii-dictionary');
		$practice_sheets = MWDB::get_practice_sheets(ASSIGNMENT_READING);
	...
	if($mode == 'homework' && !$sid) {
		wp_redirect(locale_home_url() . '/?r=reading-comprehension');
		exit;
	}
...
<?php
	$info_tab_url = get_info_tab_cloud_url('Popup_info_14.jpg');
	if($mode == 'homework') {
	...
	<div class="row"<?php echo $teacher_taking_test ? ' style="display: none"' : '' ?>>
		...
		<div class="col-sm-3 form-group">
			...
			<div class="words-pagin">
				<?php
					printf(__('Question %s of %s', 'iii-dictionary'),
	...
	<div class="row">
		...
		<div class="col-sm-12">
			<div class="form-group">
				...
				<div id="reading-passage-box" class="scroll-list" style="max-height: 450px">
					<div id="reading-passage"><?php echo $sheet->passages ?></div>
				</div>
			</div>
		</div>
		<div class="col-sm-12">
			...
			<div class="modal-dialog">
			  ...
			  <?php endif ?>
				<div class="modal-footer">
					<div class="row">				
						<div class="col-sm-12">
							<a href="<?php echo locale_home_url() ?>/?r=reading-comprehension" class="btn btn-block grey secondary"><span class="icon-switch"></span><?php _e('Go back', 'iii-dictionary') ?></a>
						</div>
					</div>
				</div>
			  </div>
      ...
      <div class="modal-content">
		...
		<div class="modal-footer">
			<div class="row">
				...
				<?php else : ?>
					<div class="col-sm-6 form-group">
						<a href="<?php echo locale_home_url() . '/?r=reading-comprehension&amp;hid=' . $sheet->next_homework_id . '&amp;ref=' . $_GET['ref'] ?>" class="btn btn-block orange confirm"><span class="icon-accept"></span><?php _e('Yes', 'iii-dictionary') ?></a>
					</div>
					<div class="col-sm-6 form-group">
...
<script>
	...
	(function($){
		...
		function setup_question(){
			var _ci = $("#current-word").val(), choices = [], selected;
			if(typeof words[_ci] != "undefined"){
