<?php
	...
	$_page_title = __('Homework Status', 'iii-dictionary');
	$_averge = 0;
...
<?php get_dict_page_title($_page_title, '', '', array(), get_info_tab_cloud_url('Popup_info_11.jpg')) ?>
	...
	<form method="post" action="<?php echo locale_home_url()?>/?r=homework-status<?php echo $gid ? '&amp;gid=' . $gid : '' ?>" id="main-form" enctype="multipart/form-data">
		...
		<?php if(!$gid) : ?>
			...
			<div class="col-sm-12">
				<h2 class="title-border"><?php _e('My Groups/Classes', 'iii-dictionary') ?></h2>
			</div>
			<div class="col-sm-12">
										...
										<?php else : ?>
											<?php foreach($user_groups->items as $item) : ?>
												<tr>
													...
													<td>
														<a href="<?php echo locale_home_url() . '/?r=homework-status&amp;gid=' . $item->group_id ?>" class="btn btn-default btn-block btn-tiny grey"><?php _e('Homeworks', 'iii-dictionary') ?></a>
													</td>
													<td><?php
														if(!$item->is_default) : ?>
      ...
      <div class="modal-content">
		...
		<?php else : ?>
			...
			<div class="col-sm-4 " style="margin-top: 20px">
				<div class="form-group">
					<a href="<?php echo locale_home_url() ?>/?r=homework-status" class="btn btn-default grey form-control"><span class="icon-goback"></span><?php _e('Go back', 'iii-dictionary') ?></a>
				</div>
			</div>
